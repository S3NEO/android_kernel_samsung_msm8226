...
#if defined(CONFIG_MACH_S3VE_CHN_CTC) || defined(CONFIG_MACH_CRATERVE_CHN_CTC) || defined(CONFIG_MACH_MS01_CHN_CTC)
	if (mt != MIGRATE_ISOLATE) {
		/* Obey watermarks as if the page was being allocated */
		watermark = low_wmark_pages(zone) + (1 << order);
		if (!is_migrate_cma(mt) && !zone_watermark_ok(zone, 0, watermark, 0, 0))
			return 0;
