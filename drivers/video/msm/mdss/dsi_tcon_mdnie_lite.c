...
int update_dsi_tcon_mdnie_register(void)
{
	struct dsi_cmd_desc *tune_data = NULL;
	...
	dsi_tcon_mdnie_tune_cmd(tune_data, ARRAY_SIZE(BYPASS_MDNIE));
	return 0;
}
			...
			const char *buf, size_t size)
{
	...
	if (cmd_value == NEGATIVE)
		...
		for(loop = 0; loop < MDNIE_COLOR_BLINDE_CMD_SIZE; loop++)
			COLOR_BLIND_MDNIE[3].payload[loop + 1] = buffer[loop];
#endif
...
void init_dsi_tcon_mdnie_class(void)
{
