...
#if defined(CONFIG_TCON_MDNIE_LITE)
...
void mdss_dsi_cmds_send(struct mdss_dsi_ctrl_pdata *ctrl, struct dsi_cmd_desc *cmds, int cnt,int flag)
{
	...
	cmdreq.cmds = cmds;
	cmdreq.cmds_cnt = cnt;
	cmdreq.flags = CMD_REQ_COMMIT | flag;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
...
#if defined(CONFIG_TCON_MDNIE_LITE)
int dsi_tcon_mdnie_tune_cmd(struct dsi_cmd_desc *tune_data, int tune_cnt)
{
	struct mdss_panel_data *pdata = msd.mpd;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata, panel_data);

	if (msd.mfd->resume_state == MIPI_RESUME_STATE) {
		mdss_dsi_cmds_send(ctrl_pdata, tune_data, tune_cnt,0);
		return true;
	} else {
		pr_info("%s suspend status", __func__);
		return false;
	}
}
#endif
...
static unsigned int mipi_samsung_manufacture_id(struct mdss_dsi_ctrl_pdata *ctrl)
{
	unsigned int id = 0 ;

	char read_reg[2] = {0xDA, 0x00 };
	struct dsi_cmd_desc manufacture_id_cmds[] = {{
		{DTYPE_DCS_READ, 1, 0, 1, 5, sizeof(read_reg)}, read_reg}, };

	mdss_dsi_cmd_receive(ctrl, manufacture_id_cmds, 1);
	pr_err("%s: manufacture_id1=%x\n", __func__, *(ctrl->rx_buf.data));

	id = (*(ctrl->rx_buf.data) & 0xFF);
	id <<= 8;

	read_reg[0] = 0xDB;
	mdss_dsi_cmd_receive(ctrl, manufacture_id_cmds, 1);
	pr_err("%s: manufacture_id2=%x\n", __func__, *(ctrl->rx_buf.data));
	id |= (*(ctrl->rx_buf.data) & 0xFF);
	id <<= 8;

	read_reg[0] = 0xDC;
	mdss_dsi_cmd_receive(ctrl, manufacture_id_cmds, 1);
	pr_err("%s: manufacture_id3=%x\n", __func__, *(ctrl->rx_buf.data));
	id |= (*(ctrl->rx_buf.data) & 0xFF);

	pr_err("%s: manufacture_id=%x\n", __func__, id);

#if 0
	read_reg[0] = 0xC3;
	mdss_dsi_cmd_receive(ctrl, manufacture_id_cmds, 3);
	pr_err("%s: 0xC3=%x 0x%x 0x%x \n", __func__, 
		*(ctrl->rx_buf.data), *(ctrl->rx_buf.data + 1), *(ctrl->rx_buf.data + 2));
#endif
	return id;
}
...
#if defined(CONFIG_TCON_MDNIE_LITE)
	update_dsi_tcon_mdnie_register();
#endif
...
#endif
	if(first_boot) {
		mipi_samsung_manufacture_id(ctrl);
		first_boot = 0;
	}
...
static struct dsi_cmd_desc mdni_tune_cmd[] = {
	{{DTYPE_DCS_LWRITE, 0, 0, 0, 0, sizeof(lock_on)}, lock_on},
	{{DTYPE_DCS_LWRITE, 0, 0, 0, 0, sizeof(mdni_tuning1)}, mdni_tuning1},
	{{DTYPE_DCS_LWRITE, 0, 0, 0, 0, sizeof(mdni_tuning2)}, mdni_tuning2},
	{{DTYPE_DCS_LWRITE, 0, 0, 0, 0, sizeof(mdni_tuning3)}, mdni_tuning3},
	{{DTYPE_DCS_LWRITE, 0, 0, 0, 0, sizeof(mdni_tuning4)}, mdni_tuning4},
	{{DTYPE_DCS_LWRITE, 0, 0, 0, 0, sizeof(mdni_tuning5)}, mdni_tuning5},
	{{DTYPE_DCS_LWRITE, 0, 0, 0, 0, sizeof(mdni_tuning6)}, mdni_tuning6},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(lock_off)}, lock_off},
};
...
static void sending_tune_cmd(char *src, int len)
{
	...
	for (data_pos = 0; data_pos < len;) {
		if (*(src + data_pos) == '0' && *(src + data_pos + 1) == 'x') {
			...
			if (cmd_step == 1 && cmd_pos == TUNE_FIRST_SIZE) {
				cmd_pos = 0;
				cmd_step = 2;
			} else if (cmd_step == 2 && cmd_pos == TUNE_SECOND_SIZE) {
				cmd_pos = 0;
				cmd_step = 3;
			} else if (cmd_step == 3 && cmd_pos == TUNE_THIRD_SIZE) {
				cmd_pos = 0;
				cmd_step = 4;
			} else if (cmd_step == 4 && cmd_pos == TUNE_FOURTH_SIZE) {
					cmd_pos = 0;
				cmd_step = 5;
			} else if (cmd_step == 5 && cmd_pos == TUNE_FIFTH_SIZE) {
				cmd_pos = 0;
				cmd_step = 6;
				}
		} else {
