...
#include "mdnie_lite_tuning_data_craterQ.h"
#endif
...
#elif defined (CONFIG_FB_MSM_MDSS_HIMAX_QHD_PANEL)
#define PAYLOAD mdni_tune_cmd[0]

#define INPUT_PAYLOAD(x) PAYLOAD.payload = x
#else
...
#elif defined (CONFIG_FB_MSM_MDSS_HIMAX_QHD_PANEL)
	DPRINT("\n");
	DPRINT("---- size1 : %d", PAYLOAD.dchdr.dlen);
	for (i = 0; i < MDNIE_TUNE_FIRST_SIZE ; i++)
		DPRINT("0x%x ", PAYLOAD.payload[i]);
	DPRINT("\n");
#else
	DPRINT("\n");
...
#endif
		case mDNIe_VIDEO_MODE:
			DPRINT(" = VIDEO_MDNIE MODE =\n");
			INPUT_PAYLOAD(VIDEO_MDNIE);
			break;
		case mDNIe_UI_MODE:
			DPRINT(" = UI_MDNIE MODE ..=\n");
			INPUT_PAYLOAD(UI_MDNIE);
			break;
		case mDNIe_GALLERY:
			DPRINT(" = GALLERY_MDNIE MODE =\n");
			INPUT_PAYLOAD(GALLERY_MDNIE);
			break;
		case mDNIe_CAMERA_MODE:
			DPRINT(" = CAMERA_MDNIE MODE =\n");
			INPUT_PAYLOAD(CAMERA_MDNIE);
			break;
		case mDNIE_BLINE_MODE:
			DPRINT(" = COLOR_BLIND MDNIE =\n");
			INPUT_PAYLOAD(COLOR_BLIND_MDNIE);
			break;
		case mDNIe_VIDEO_WARM_MODE:
			DPRINT(" = VIDEO WARM MODE =\n");
			DPRINT("no data for WARM MODE..\n");
