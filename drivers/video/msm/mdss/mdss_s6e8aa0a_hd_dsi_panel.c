...
struct delayed_work  err_fg_work;
static int err_fg_gpio;
static int esd_count;
static volatile int esd_irq_on;
int err_fg_working;
int disp_esd_gpio;
#define ESD_DEBUG 1
#endif
...
static irqreturn_t err_fg_irq_handler(int irq, void *handle)
{
	if(esd_irq_on) {
		...
		schedule_delayed_work(&err_fg_work, 100);
...
//extern void mdss_fb_pan_idle(struct msm_fb_data_type *mfd);
//extern void mdss_fb_release_fences(struct msm_fb_data_type *mfd);
...
static void err_fg_work_func(struct work_struct *work)
{
	...
	if((mfd->resume_state == MIPI_SUSPEND_STATE) || (!mfd->panel_power_on)){
		pr_err("%s: Display off Skip ESD recovery\n", __func__);
		return;
	}

	rc = gpio_get_value(disp_esd_gpio);
	if(rc == 1)
	{
		esd_irq_on = true;
		enable_irq(err_fg_gpio);
		pr_err("%s: ESD GPIO High, Skip ESD recovery\n", __func__);
		return;
	}
	...
	//QC suggest to power on/off panel only
	//mdss_fb_pan_idle(mfd);

	//mdss_mdp_rotator_unset(MDSS_MDP_ROT_SESSION_MASK);

	mdss_mdp_ctl_intf_event(msdCtl, MDSS_EVENT_PANEL_OFF, NULL);
	mdelay(100);
	//mdss_fb_release_fences(mfd);
	mdss_mdp_ctl_intf_event(msdCtl, MDSS_EVENT_UNBLANK, NULL);

/*
	msdCtl->stop_fnc(msdCtl);
	mdelay(100);
	msdCtl->start_fnc(msdCtl);
*/
/*
	mdss_dsi_panel_off(pdata);
	mdelay(10);
	mdss_dsi_s6e8aa0a_panel_reset(pdata, 0);
	mdelay(100);
	mdss_dsi_s6e8aa0a_panel_reset(pdata, 1);
	mdelay(100);
...
#if defined(CONFIG_ESD_ERR_FG_RECOVERY)
					INIT_DELAYED_WORK(&err_fg_work, err_fg_work_func);
