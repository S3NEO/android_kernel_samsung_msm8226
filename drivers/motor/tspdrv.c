...
static int tspdrv_parse_dt(struct platform_device *pdev)
{
	...
	vibrator_drvdata.vib_pwm_gpio = of_get_named_gpio(np, "samsung,vib_pwm_gpio", 0);	
...
#if 0//def CONFIG_MOTOR_DRV_MAX77803
...
static int32_t drv2603_gpio_init(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(vibrator_drvdata.drv2603_en_gpio,
...
#else
void drv2603_regulator_en(bool en)
{
	int ret;
	static struct regulator *reg_l23;
	...
	if (!reg_l23) {
		...
		if (IS_ERR(reg_l23)) {
			printk(KERN_ERR"could not get 8226_l23, rc = %ld\n",
				PTR_ERR(reg_l23));
			return;
		}
	}
