...
*/
	if(!ldo19){
		...
		pr_info("[TMP] %s, %d\n", __func__, __LINE__);
		if (rc){
			printk(KERN_ERR "%s: taos set_level failed 8226_l19 (%d)\n",__func__, rc);
		}
	}

	if(!lvs1){
		lvs1 = regulator_get(NULL,"8226_l6");
		rc = regulator_set_voltage(lvs1,1800000,1800000);
		pr_info("[TMP] %s, %d\n", __func__, __LINE__);
		if (rc){
			printk(KERN_ERR "%s: taos set_leve 8226_l6l failed (%d)\n",__func__, rc);
		}
	}
	if(en){
/*
...
#if defined(CONFIG_MACH_Q7_CHN_SGLTE)
	if(system_rev == 0) {
		gpio_free(pdata->en_2nd);
		ret = gpio_request(pdata->en_2nd, "prox_en_2nd");
		if(ret)
			pr_err("[taos]%s: en_2nd gpio request fail\n",__func__);
		gpio_tlmm_config(GPIO_CFG(pdata->en_2nd, 0,
				GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
		if (ret) {
			pr_err("[taos]%s: unable to request prox_en_2nd [%d]\n",
					__func__, pdata->en_2nd);
			return;
		}
		ret = gpio_direction_output(pdata->en_2nd, 1);
		if (ret)
			pr_err("[taos]%s: unable to set_direction for prox_en_2nd [%d]\n",__func__, pdata->en_2nd);
