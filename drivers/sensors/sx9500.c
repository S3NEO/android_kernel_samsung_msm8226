...
static void send_event(struct sx9500_p *data, int cnt, u8 state)
{
	...
	switch (cnt) {
		...
		case 2:
		case 3:
			pr_err("[SX9500]: %s - There is no defined event for"
				" button %d.\n", __func__, cnt);
			break;
		default:
			break;
...
static s32 sx9500_get_capMain(struct sx9500_p *data)
{
	...
	pr_err("[SX9500]: %s - CapsMain: %ld, useful: %ld, Offset: %u\n",
...
static void sx9500_open_caldata(struct sx9500_p *data)
{
	...
	if (IS_ERR(cal_filp)) {
		...
		if (ret != -ENOENT)
			...
			pr_err("[SX9500]: %s - There is no calibration file\n",
				__func__);
...
static int sx9500_set_mode(struct sx9500_p *data, unsigned char mode)
{
	...
	pr_err("[SX9500]: %s - change the mode : %u\n", __func__, mode);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_err("[SX9500]: %s - normal threshold %lu\n", __func__, val);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sysfs_streq(buf, "1"))
		...
		pr_err("[SX9500]: %s - invalid value %d\n", __func__, *buf);
		return -EINVAL;
	}
	...
	for (cnt = 0; cnt < TOTAL_BOTTON_COUNT; cnt++) {
		if (data->state[cnt] == IDLE) {
			if (status & (CSX_STATUS_REG << cnt))
				...
				pr_err("[SX9500]: %s - %d already released.\n",
					__func__, cnt);
		} else { /* User released button */
			if (!(status & (CSX_STATUS_REG << cnt))) {
				if ((data->touchMode == INIT_TOUCH_MODE)
					&& (capMain >= threshold))
					pr_err("[SX9500]: %s - IDLE SKIP\n",
						__func__);
				else
					send_event(data, cnt, IDLE);

			} else {
				pr_err("[SX9500]: %s - %d still touched\n",
					__func__, cnt);
			}
		}
	}
...
static void sx9500_initialize_variable(struct sx9500_p *data)
{
	...
	pr_err("[SX9500]: %s - Init Touch Threshold : %d\n",
	...
	pr_err("[SX9500]: %s - Normal Touch Threshold : %u\n",
...
int sx9500_power_on(struct sx9500_p *data, bool onoff)
{
	...
	if (!data->lvs1_1p8) {
		data->lvs1_1p8 = regulator_get(&data->client->dev, "8226_l6");
		if(!data->lvs1_1p8){
		...
		if (ret) {
			pr_err("%s: set voltage failed on l6_1p8, rc=%d\n",
				__func__, ret);
			return ret;
		}
	}
...
{
	pr_err("[SX9500]  %s is called /n" ,__func__);
