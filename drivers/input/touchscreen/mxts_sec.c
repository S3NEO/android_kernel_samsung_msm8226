	...
	struct mxt_object *dbg_object, u8 dbg_mode, u8 read_point, u16 num)
{
	...
	if (dbg_mode == MXT_DIAG_DELTA_MODE) {
		...
		if(data->charging_mode){
			if((num / data->fdata->num_ynode)==18)
				goto skip;
		}
...
#if 0	/* Need to be removed hard cording in date */
...
static void run_reference_read(void *device_data)
{
	...
	if(data->charging_mode){
		mxt_write_object(data, MXT_TOUCH_MULTITOUCHSCREEN_T9, 6, 80);
		printk("[TSP] %s, T9=80(0x50)\n",__func__);
	}
