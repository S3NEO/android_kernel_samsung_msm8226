...
#undef pr_err
#undef pr_info
#define dev_dbg(dev, format, arg...)  zinitix_printk(format, ##arg)
#define dev_err(dev, format, arg...)   zinitix_printk(format, ##arg)
#define dev_info(dev, format, arg...)   zinitix_printk(format, ##arg)
#define pr_err(fmt, ...) zinitix_printk(fmt, ##__VA_ARGS__)
#define pr_info(fmt, ...)  zinitix_printk(fmt, ##__VA_ARGS__)
/* added header file */
...
static void ts_tmr_work(struct work_struct *work)
{
	...
	if(down_trylock(&info->work_lock)) {
		dev_err(&info->client->dev, "%s: Failed to occupy work lock\n", __func__);
	...
	dev_info(&info->client->dev, "tmr queue work\n");
#endif

	return;

fail_time_out_init:
	dev_err(&info->client->dev, "Failed to restart\n");
	esd_timer_start(CHECK_ESD_TIMER, info);
	info->work_state = NOTHING;
	enable_irq(info->irq);
	up(&info->work_lock);
		...
		u16 version, u16 minor_version, u16 reg_version, u16 hw_id)
{
	...
	dev_err(&info->client->dev, "HW_ID = 0x%x, new HW_ID = 0x%x\n",
				hw_id, new_hw_id);
	if (hw_id != new_hw_id)
		return false;
#endif
...
#if defined(TSP_VERBOSE_DEBUG)
	dev_err(&info->client->dev, "version = 0x%x, new version = 0x%x\n",
				version, new_version);
#endif
...
#if defined(TSP_VERBOSE_DEBUG)
	dev_err(&info->client->dev, "minor version = 0x%x, new minor version = 0x%x\n",
	...
	dev_err(&info->client->dev, "reg data version = 0x%x, new reg data"
...
static void clear_report_data(struct bt532_ts_info *info)
{
	...
	for (i = 0; i < info->cap_info.button_num; i++) {
		if (info->button[i] == ICON_BUTTON_DOWN) {
			...
			dev_info(&info->client->dev, "Button up = %d\n", i);
#endif
		}
	}
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
			...
			reported = true;
			dev_info(&info->client->dev, "Finger [%02d] up\n", i);
		}
...
//			input_report_abs(info->input_dev, ABS_MT_PALM, 1);
			...
			if (zinitix_bit_test(sub_status, SUB_BIT_DOWN)) {
				...
				dev_info(&client->dev, "Finger [%02d] down. x = %d, y = %d,"
							" w = %d\n", i, x, y, w);
#else
...
static int bt532_ts_resume(struct device *dev)
{
	...
	zinitix_printk( " called\n");
	down(&info->work_lock);
	if (info->work_state != SUSPEND) {
		dev_err(&info->client->dev, "%s: Invalid work proceedure (%d)\n",
				__func__, info->work_state);
		up(&info->work_lock);
	...
	if (mini_init_touch(info) == false)
		dev_err(&info->client->dev, "Failed to resume\n");
...
#if defined(TSP_VERBOSE_DEBUG)
	dev_info(&info->client->dev, "resume\n");
#endif
	up(&info->work_lock);
...
static void fw_update(void *device_data)
{
	...
	case UMS:
		...
		if (IS_ERR(fp)) {
			dev_err(&info->client->dev, "Failed to open %s (%d)\n", fw_path, (s32)fp);
			info->factory_info->cmd_state = FAIL;
		...
		if(fsize != info->cap_info.ic_fw_size) {
			dev_err(&info->client->dev, "Invalid fw size!!\n");
			info->factory_info->cmd_state = FAIL;
	...
	default:
		dev_err(&info->client->dev, "Invalid fw file type!!\n");
				  ...
				  *devattr, const char *buf, size_t count)
{
	...
	if (finfo->cmd_is_running == true) {
		dev_err(&info->client->dev, "%s: other cmd is running\n", __func__);
	...
	dev_info(&info->client->dev, "cmd = %s\n", tsp_cmd_ptr->cmd_name);
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(&info->client->dev, "%s: key threshold = %d \n", __func__, cap->key_threshold);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case TOUCH_IOCTL_HW_CALIBRAION:
		...
		if (write_reg(misc_info->client,
			...
			zinitix_printk("failed to set touch mode %d.\n",
				mode);
			goto fail_hw_cal;
		}
...
void zinitix_vdd_on(struct bt532_ts_info *info,bool onoff)
{
	...
	if(onoff){
		if (info->vddldo_vreg)
			regulator_enable(info->vddldo_vreg);
	}else{
		if (info->vddldo_vreg)
			regulator_disable(info->vddldo_vreg);
	}
#else
	if(onoff){
		if (info->vddo_vreg)
...
#if defined(CONFIG_MACH_MS01_LTE) || defined(CONFIG_MACH_Q7_CHN_SGLTE)
	if(onoff)
	{
		...
		const struct i2c_device_id *i2c_id)
{
	...
	regulator_enable(info->vddo_vreg);		// always on. i2c pull up

	info->vddldo_vreg = regulator_get(&info->client->dev,"vddldo");
	if (IS_ERR(info->vddldo_vreg)){
		info->vddldo_vreg = NULL;
		zinitix_printk("info->vddldo_vreg error\n");
		ret = -EPERM;
		goto err_power_sequence;
	}
#endif	
...
static int __devinit bt532_ts_init(void)
{
	if (poweroff_charging) {
		zinitix_printk("LPM Charging Mode!!\n");
		return 0;
	}
	else
