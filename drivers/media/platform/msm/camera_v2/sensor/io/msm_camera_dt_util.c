	...
	for (i = 0; i < count; i++) {
		...
		case SENSOR_GPIO:
			if (!strcmp(seq_name, "sensor_gpio_reset"))
				...
				ps[i].seq_val = SENSOR_GPIO_EXT_CAMIO_EN;
			else if (!strcmp(seq_name, "qcom,gpio-ext-camio-en-vt"))
				ps[i].seq_val = SENSOR_GPIO_EXT_CAMIO_EN_VT;
#else				
			else if (!strcmp(seq_name, "sensor_gpio_ext_camio_en"))
				ps[i].seq_val = SENSOR_GPIO_EXT_CAMIO_EN;
#endif
	...
	uint16_t gpio_array_size)
{
	...
	if (of_property_read_bool(of_node, "qcom,gpio-ext-camio-en-vt") == true) {
			...
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_EXT_CAMIO_EN_VT] = 
				gpio_array[val];
			pr_err("%s qcom,gpio-ext-camio-en-vt %d\n", __func__,
					gconf->gpio_num_info->gpio_num[SENSOR_GPIO_EXT_CAMIO_EN_VT]);
			pr_err("%s val %d\n", __func__,
					val);			
	}
...
#endif
#if defined(CONFIG_MACH_MILLET3G_EUR) || defined(CONFIG_MACH_HLITE_CHN_SGLTE)
	if (of_property_read_bool(of_node, "qcom,gpio-vt-reset") == true) {
			rc = of_property_read_u32(of_node, "qcom,gpio-vt-reset", &val);
			if (rc < 0) {
				pr_err("%s:%d read qcom,gpio-vt-reset  rc %d\n",
...
#ifdef CONFIG_MACH_CRATERVE_CHN_CTC
