...
#include <media_tab4/msm_cam_sensor.h>
...
static int32_t hvca_read(uint8_t addr, uint8_t *data)
{
	...
	af_client->cci_client = 0;
	rc = msm_camera_cci_i2c_read(af_client, addr, &buf, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0) {
...
static int32_t hvca_write(uint8_t addr, uint8_t data)
{
	...
	af_client->cci_client = 0;
	rc = msm_camera_cci_i2c_write(af_client, addr, data, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0) {
...
static int32_t hvca_write_w(uint8_t addr, uint16_t data)
{
	...
	rc = msm_camera_cci_i2c_write(af_client, addr, data, MSM_CAMERA_I2C_WORD_DATA);
	if (rc < 0) {
...
uint16_t inf_dac1, inf_dac2, start_dac, macro_dac1, macro_dac2;
/*End : shchang@qti.qualcomm.com - 20130312 */
uint16_t cal_offset = 0;//SEMCO Request by Lizk 05112013
...
/*End : shchang@qti.qualcomm.com - 20130312 */
	...
	position_inf1 = 852, position_inf2 = 651, position_mac1 = 160, position_mac2 = 400, position_pan = 659;
...
void hvca_actuator_set_inf_focus(int value){
	hvca_lensMovePulse(value);
}
...
int32_t hvca_actuator_move_focus(
	...
	struct msm_actuator_move_params_t *move_params)
{
	int32_t new_step_position_table;
	int32_t offset = 0;
/*Start : shchang@qti.qualcomm.com - 20130312 */
