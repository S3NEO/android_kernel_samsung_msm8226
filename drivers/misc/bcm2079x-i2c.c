...
#if defined (CONFIG_MACH_VICTOR_CHN_SGLTE) || defined(CONFIG_MACH_VICTOR_CHN_SGLTE_CU) || defined(CONFIG_MACH_Q7_CHN_SGLTE)
	struct regulator *L6;
#endif
...
#if defined (CONFIG_MACH_VICTOR_CHN_SGLTE) || defined(CONFIG_MACH_VICTOR_CHN_SGLTE_CU) || defined(CONFIG_MACH_Q7_CHN_SGLTE)
int bcm2079x_power_onoff(struct bcm2079x_dev *bcm2079x_dev, int onoff)
{
	int ret = -1;
	if (!bcm2079x_dev->L6) {
		bcm2079x_dev->L6= regulator_get(&bcm2079x_dev->client->dev, "8226_l6");
		if (!bcm2079x_dev->L6) {
			pr_err("%s: regulator pointer null L6, rc=%d\n",
				__func__, ret);
			return ret;
		}
		ret = regulator_set_voltage(bcm2079x_dev->L6, 1800000, 1800000);
		if (ret) {
			pr_err("%s: set voltage failed on L6, rc=%d\n",
				__func__, ret);
			return ret;
		}
	}
	...
	if(onoff){
		ret = regulator_enable(bcm2079x_dev->L6);
		if (ret) {
		...
		ret = regulator_disable(bcm2079x_dev->L6);
		if (ret) {
...
#if defined (CONFIG_MACH_VICTOR_CHN_SGLTE) || defined(CONFIG_MACH_VICTOR_CHN_SGLTE_CU) || defined(CONFIG_MACH_Q7_CHN_SGLTE)
	bcm2079x_power_onoff(bcm2079x_dev,1);
#endif
